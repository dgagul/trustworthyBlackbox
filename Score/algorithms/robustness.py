...
def analyse(model, train_data, test_data, config, factsheet):
    ...
    output = dict(
        ...
        # feature takes exponentially long to compute, for RF more than 3h on diabetes dataset
...
def clique_method(model, train_data, test_data, thresholds, factsheet):
    ...
    Returns:
        ...
        Error
    """
    with open('Score/configs/mappings/robustness/default.json', 'r') as f:
    ...
    if thresholds == default_map["score_clique_method"]["thresholds"]["value"]:
        if "scores" in factsheet.keys() and "properties" in factsheet.keys():
            # return result(score=int(3), properties={
            #     "error_bound": info("Average error bound", "{:.2f}".format(0.015)),
            #     "error": info("Error", "{:.1f}".format(0.1)),
            #     "depends_on": info("Depends on", "Model")
            # })
